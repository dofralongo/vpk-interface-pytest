...
@allure.epic("百迈接口测试")
@allure.feature("登录模块")
class TestQueryexposureroi:

    @allure.story("登录功能测试")
    @pytest.mark.parametrize('in_data', eval(re_data), ids=[i['detail'] for i in TestData])
    def test_QueryExposureRoi(self, in_data, case_skip):
        """
        :param :
        :return:
        """
...
if __name__ == '__main__':
    pytest.main(['test_test_QueryExposureRoi.py', '-s', '-W', 'ignore:Module already imported:pytest.PytestWarning'])
