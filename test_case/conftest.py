...
def work_login_init():
    ...
    url = "http://127.0.0.1:11111/account/login"
    data = { }
    headers = {'username': 'administrator',"password":"123456"}
    # 请求登录接口

    res = requests.get(url=url, data=data, verify=True, headers=headers)
    # print(res.text)

    # response_token = json.loads(res.text)['session_id'] + ' ' + json.loads(res.text)['access_token']
    response_token = json.loads(res.text)['session_id']
    CacheHandler.update_cache(cache_name='get_session_id', value=response_token)
