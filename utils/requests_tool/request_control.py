...
class RequestControl:
        ...
        if _data is not None:
            ...
            for key, value in _data.items():
                if value is None or value == '':
                    ...
                    # 修改params的拼接 按中台接口文档生成样式如下：/sysUser/{id}
                    params_data += ('/'+ str(value) + "&")
    ...
    def request_type_for_data(
        ...
        # for data in self.__yaml_case.data:
        # for data_key,data_value in self.__yaml_case.data:
        data = self.__yaml_case.data
        _data, _headers = self.multipart_in_headers(
            ast.literal_eval(cache_regular(str(data))),
            # ast.literal_eval(cache_regular(str(data_value))),
            headers
        )
        _url = self.__yaml_case.url
        res = requests.request(
